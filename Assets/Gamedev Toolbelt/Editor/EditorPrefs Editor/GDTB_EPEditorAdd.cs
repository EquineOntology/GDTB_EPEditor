...
public class GDTB_EPEditorAdd : EditorWindow
{
    private string _key = "";
    private int _type = 0;
    ...
    {
        GDTB_EPEditorAdd window = (GDTB_EPEditorAdd)EditorWindow.GetWindow(typeof(GDTB_EPEditorAdd));
    ...
    public void OnEnable()
    {
        ...
        _GDTBSkin = Resources.Load(GDTB_EPConstants.FILE_GUISKIN, typeof(GUISkin)) as GUISkin;
    }
            ...
            else
            {
                if (EditorUtility.DisplayDialog("Add editor preference?", "Are you sure you want to add this key to EditorPrefs?", "Add key", "Cancel"))
                {
                    ...
                    if (EditorPrefs.HasKey(_key))
                    {
                        ...
                        {
                            case 0:
                                GDTB_EPEditor.Prefs.Add(new GDTB_EditorPref((GDTB_EditorPrefType)_type, _key, _boolValue.ToString()));
                                break;
                            case 1:
                                GDTB_EPEditor.Prefs.Add(new GDTB_EditorPref((GDTB_EditorPrefType)_type, _key, _intValue.ToString()));
                                break;
                            case 2:
                                GDTB_EPEditor.Prefs.Add(new GDTB_EditorPref((GDTB_EditorPrefType)_type, _key, _floatValue.ToString()));
                                break;
                            case 3:
                                GDTB_EPEditor.Prefs.Add(new GDTB_EditorPref((GDTB_EditorPrefType)_type, _key, _stringValue));
                                break;
                        }
                        EditorWindow.GetWindow(typeof(GDTB_EPEditorAdd)).Close();
