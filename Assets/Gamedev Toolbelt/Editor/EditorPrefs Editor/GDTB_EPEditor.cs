...
public class GDTB_EPEditor : EditorWindow
{
    public static List<GDTB_EditorPref> Prefs = new List<GDTB_EditorPref>();
    ...
    public static void Init()
    {
        ...
        var window = (GDTB_EPEditor)EditorWindow.GetWindow(typeof(GDTB_EPEditor));
        ...
        var storedPrefs = GDTB_EPEditorIO.LoadStoredPrefs();
        if (storedPrefs.Count >= Prefs.Count)
        {
            GDTB_EPEditor.Prefs.Clear();
            GDTB_EPEditor.Prefs.AddRange(storedPrefs);
        }
    ...
    private void DrawPrefs()
    {
        ...
        for (var i = 0; i < Prefs.Count; i++)
        {
            ...
            var helpBoxHeight = keyHeight + valHeight + GDTB_EPConstants.LINE_HEIGHT + 5;
    ...
    private void DrawType(Rect aRect, GDTB_EditorPref aPref)
    {
        var typeRect = aRect;
    ...
    private void DrawKeyAndValue(Rect aRect, GDTB_EditorPref aPref, float aHeight)
    {
        // Key.
        var keyRect = aRect;
    ...
    private void DrawEditAndRemoveButtons(Rect aRect, GDTB_EditorPref aPref)
    {
        // "Edit" button.
        var editRect = aRect;
        ...
        var editButton = new GUIContent(Resources.Load(GDTB_EPConstants.FILE_GDTB_EDIT, typeof(Texture2D)) as Texture2D, "Edit this EditorPref");
        ...
        var removeButton = new GUIContent(Resources.Load(GDTB_EPConstants.FILE_GDTB_REMOVE, typeof(Texture2D)) as Texture2D, "Remove this EditorPref");
        ...
        if (GUI.Button(removeRect, removeButton))
        {
            ...
            if (EditorUtility.DisplayDialog("Remove EditorPref", "Are you sure you want to delete this EditorPref?", "Delete pref", "Cancel"))
            {
                ...
                Prefs.Remove(aPref);
                EditorWindow.GetWindow(typeof(GDTB_EPEditor)).Repaint();
            }
        }
    }
    ...
    private void DrawAddButton()
    {
        ...
        var addButton = new GUIContent(Resources.Load(GDTB_EPConstants.FILE_GDTB_ADD, typeof(Texture2D)) as Texture2D, "Add a new key");

        // Add QQQ on click.
        if (GUI.Button(addRect, addButton))
        {
            GDTB_EPEditorAdd.Init();
        }
    }
    ...
    private void DrawGetButton()
    {
        ...
        var getButton = new GUIContent(Resources.Load(GDTB_EPConstants.FILE_GDTB_ADD, typeof(Texture2D)) as Texture2D, "Add existing key");

        // Get value and type on click.
        if (GUI.Button(getRect, getButton))
        {
            GDTB_EPEditorGet.Init();
        }
    }
    ...
    {
        GDTB_EPEditorIO.WritePrefsToFile();
    }
}