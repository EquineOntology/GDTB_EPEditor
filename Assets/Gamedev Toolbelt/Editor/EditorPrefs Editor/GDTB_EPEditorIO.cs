...
public static class GDTB_EPEditorIO
{
    ...
    public static void WritePrefsToFile()
    {
        ...
        try
        {
            foreach (var pref in GDTB_EPEditor.Prefs)
            {
    ...
    public static List<GDTB_EditorPref> LoadStoredPrefs()
    {
        var backedPrefs = new List<GDTB_EditorPref>();
    ...
    private static GDTB_EditorPref ParsePref(string aString)
    {
        ...
        GDTB_EditorPrefType type;
        bool boolValue;
        int intValue;
        float floatValue;
        string stringValue;

        GDTB_EditorPref pref;
        switch (typeString)
        {
            case "BOOL":
                type = GDTB_EditorPrefType.BOOL;
                boolValue = UnityEditor.EditorPrefs.GetBool(key, false);
                pref = new GDTB_EditorPref(type, key, boolValue.ToString());
                break;
            case "INT":
                type = GDTB_EditorPrefType.INT;
                intValue = UnityEditor.EditorPrefs.GetInt(key, 0);
                pref = new GDTB_EditorPref(type, key, intValue.ToString());
                break;
            case "FLOAT":
                type = GDTB_EditorPrefType.FLOAT;
                floatValue = UnityEditor.EditorPrefs.GetFloat(key, 0.0f);
                pref = new GDTB_EditorPref(type, key, floatValue.ToString());
                break;
            case "STRING":
            default:
                type = GDTB_EditorPrefType.STRING;
                stringValue = UnityEditor.EditorPrefs.GetString(key, "");
                pref = new GDTB_EditorPref(type, key, stringValue);
                break;
        }
        return pref;
    }
